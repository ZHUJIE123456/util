(function() {
  var sencond = ``
  for(i = 0; i < 60; i++){
    var onediv = `<div id = "sencond${i+1}" style="font-size: 16px;width: 100%;text-align: right;position: absolute;display: inline-block;transform: rotate(${(i-1)*-6}deg);"> ${i+1} 秒</div>`
  ...
  var minute = ``
  for(i = 0; i < 60; i++){
    var onediv = `<div id = "minute${i+1}"  style="font-size: 16px;width: 100%;text-align: right;position: absolute;display: inline-block;transform: rotate(${(i)*-6}deg);"> ${i+1} 分</div>`
  ...
  var hour = ``
  for(i = 0; i < 24; i++){
    var onediv = `<div id = "hour${i+1}" style="font-size: 16px;width: 100%;text-align: right;position: absolute;display: inline-block;transform: rotate(${(i)*-15}deg);"> ${i+1} 时</div>`
    hour = hour + onediv
  }
  ...
  var day = ``
  for(i = 0; i < 31; i++){
    var onediv = `<div id = "day${i+1}" style="font-size: 16px;width: 100%;text-align: right;position: absolute;display: inline-block;transform: rotate(${(i)*-11.25}deg);"> ${i+1} 日</div>`
    day = day + onediv
  }
  ...
  var mounth = ``
  for(i = 0; i < 12; i++){
    var onediv = `<div id = "mounth${i+1}" style="font-size: 16px;width: 100%;text-align: right;position: absolute;display: inline-block;transform: rotate(${(i)*-30}deg);"> ${i+1} 月</div>`
  ...
  function transformBox(){
    ...
    var sencond = nowDate.getSeconds()
    ...
    var nowDates = document.querySelectorAll('.NowData')
